...
OptionParser.new do |parser|
  ...
  parser.on('-d', '--dry-run', "Calculate the number of files and amount of data to copy, but do not actually copy anything.") do |dry_run_arg|
    options[:dry_run] = dry_run_arg
  end
...
create_initial_directories library unless options[:dry_run]
...
artists_to_sync.each do |artist|
  ...
  artist.albums.each do |album|
    ...
    files_to_sync.each do |file_to_sync|
      ...
      file_count += 1
      collection_size += File.size track_source
      unless options[:dry_run]
    ...
    unless options[:dry_run]
      if Dir.empty? target_directory
        ...
        Dir["#{source_directory}/*.jpg"].each do |image|
          collection_size += File.size image
          FileUtils.cp image, target_directory
        end
      end
    end
  end
  FileUtils.rm_r(artist_path) if !options[:dry_run] && Dir.empty?(artist_path)
end
...
puts "\n"
puts '======================================================='
puts "#{file_count} music files; #{collection_size / 1024 / 1024 } MB (including art)."
